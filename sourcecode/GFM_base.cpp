...
)
{
	...
	if (fabs(lsval) > ds) lsval = std::copysign(ds, lsval);
...
void GFM_base :: get_interpolated_mixedRPstates_VOF
(
	const sim_info& params, 
	const grideuler2type& prims1, 
	const grideuler2type& prims2,
...
{
	Eigen::Vector2d sampleposL = interfacelocation + ds * normal;
	Eigen::Vector2d sampleposR = interfacelocation - ds * normal;
